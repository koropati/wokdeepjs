...
export function erode(image, kernel) {
    const result = [];
    ...
    for (let i = 0; i < rows; i++) {
        ...
        for (let j = 0; j < cols; j++) {
            let min = 255; // Inisialisasi nilai minimum
            for (let m = 0; m < kernel.length; m++) {
                for (let n = 0; n < kernel[m].length; n++) {
                    if (kernel[m][n]) {
                        const pixelValue = image[i + m][j + n] || 0;
                        if (pixelValue < min) {
                            min = pixelValue;
                        }
                    }
...
export function dilate(image, kernel) {
    const result = [];
    const rows = image.length;
    const cols = image[0].length;
    ...
    for (let i = 0; i < rows; i++) {
        result[i] = [];
        for (let j = 0; j < cols; j++) {
            let max = 0; // Inisialisasi nilai maksimum
            for (let m = 0; m < kernel.length; m++) {
                for (let n = 0; n < kernel[m].length; n++) {
                    if (kernel[m][n]) {
                        const pixelValue = image[i + m][j + n] || 0;
